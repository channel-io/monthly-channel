...
### [React Summit 참관기 - Design System, SSR, MSA](https://channel.io/ko/blog/distributedlock_2022_backend)
...
- Notion 이 자사의 PostgreSQL 모놀리스를 샤딩한 과정에 대해 소개한다.
...
- DynamoDB 에서 Couchbase 로 마이그레이션한 배경과 가용성을 높이기 위한 과정 등을 소개한다.

### [카카오 개발자들을 위한 공용 Message Streaming Platform – Kafka & RabbitMQ](https://tech.kakao.com/2021/12/23/kafka-rabbitmq/)
- 다양한 서비스에 대한 모니터링와 운영에 대한 리소스를 줄이기 위해 공용 메세지큐 시스템을 도입한 과정을 소개한다.
...
- exactly-once 와 같은 이슈를 해결하기 위해 Kafka Streams 을 활용한 방법에 대해 소개한다.
...
- DynamoDB 와 Go 를 이용하는 서비스의 응답 속도를 높이기 위해 aws 서비스와 Go의 통신 방법에 대해 깊게 설명하고, 개선을 위해 시도한 방법들에 대해 소개한다.
...
- 쿠키런에서 CockroachDB 와 AWS 이슈에 대응한 긴급 점검 회고 영상이다.

### [Rethinking Task Size in SRE](https://www.oreilly.com/content/rethinking-task-size-in-sre/)
- 구글 SRE 팀이 서비스를 왜 스케일 아웃 대신 스케일 업을 선택했는지를 설명하고 있다. 효율성을 정량적으로 평가 하기 위해 측정항목을 정의하였고 25% 효율을 냈다고 한다.

### [“DevOps”의 인적 확장 (The human scalability of “DevOps”)](https://tech.devsisters.com/posts/the-human-scalability-of-devops/)
- 운영 인력 확장 문제 해결을 위해 DevOps 스타일의 애자일한 개발 및 자동화의 필요성과 제품 팀 내의 SRE팀 만들기 등의 투자가 필요하다고 소개한다.

### [리멤버는 서비스 모니터링을 어떻게 하고 있을까?](https://blog.dramancompany.com/2022/06/how-remember-monitors/)
- 리멤버에서 사용하고 있는 도구를 소개하고 체크하고 있는 지표들, 그리고 모니터링 방식을 소개하고 있다.

# ETC

### [Testing, Oh my!](https://jwchung.github.io/testing-oh-my)
- 어떤 코드가 테스트하기 쉬운지 또는 어려운지, 테스트가 쉬운 코드는 어떻게 작성하는지, 테스트가 어려운 코드는 어떻게 다뤄야 하는지, 그리고 테스트하기 쉬운 코드와 어려운 코드가 만나게 되는 지점은 어떤 테스트 방법이 있는지에 대해 소개한다.
